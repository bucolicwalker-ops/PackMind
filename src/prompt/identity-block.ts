...
export function buildIdentityBlock(
	...
	lines.push(
		`你是 ${config.breedName}/${config.nickname}（${config.breedName}）。`,
	);
...
export function buildTeammateRoster(myDogId: string): string {
	...
	const teammates: DogConfig[] = Object.values(allConfigs).filter(
		(cfg) => (cfg as DogConfig).id !== myDogId,
	) as DogConfig[];
	...
	for (const t of teammates) {
		...
		lines.push(
			`| ${handle} | ${t.breedName} | ${t.roleDescription} | ${t.defaultModel} | ${strengths} | ${caution} |`,
		);
	}
 ...
 */
export function buildCvoRef(coCreator: {
	name: string;
 ...
 */
export function buildWorkflowTriggers(
	breedName: string,
	_dogId: string,
	_displayName: string,
): string {
	...
	if (breedName === "德牧") {
		...
		lines.push("### 德牧家族治理");
		lines.push(
			"commit/PR title 含 fix:/hotfix:/quick fix/workaround → 归类 hotfix。",
		);
