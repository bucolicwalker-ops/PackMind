...
import { ThreadId } from '../types/thread.js';
...
class BallTracker {
  ...
  hold(threadId: ThreadId, dogId: DogId, reason: string, nextStep: string, wakeAfterMs: number): BallState {
    // Validate: only the current holder can hold
    const current = this.get(threadId);
    if (current && current.holder !== dogId) {
      throw new Error(`Ball deadlock: ${current.holder} holds the ball, ${dogId} tried to hold it`);
    }
  ...
  transfer(threadId: ThreadId, fromDogId: DogId, toDogId: DogId, reason: string): BallState {
    const current = this.get(threadId);
    if (current && current.holder !== fromDogId) {
      throw new Error(`Ball transfer conflict: ${current.holder} holds the ball, ${fromDogId} tried to transfer it`);
    }
