...
import { createThreadId, type ThreadId } from "../types/thread.js";
...
const searchEvidenceSchema = {
	query: z
		...
		.min(1)
		.describe(
			'Search query — keywords to find in conversation history (e.g. "数据库设计", "review 结果", "架构讨论")',
		),
	threadId: z
		.string()
		.optional()
		.describe(
			"Optional: restrict search to a specific thread. Omit to search across all threads.",
		),
	limit: z
		.number()
		.min(1)
...
async function handleBallState(
	args: Record<string, unknown>,
): Promise<ToolResult> {
	...
	if (!state || !state.holder) {
		return successResult(
			`Ball state for thread ${threadIdStr}: 球在地上 (no holder).`,
		);
	}
...
function searchMessages(
	...
	const matches = scored
		.filter((s) => s.score > 0)
		.sort(
			(a, b) => b.score - a.score || b.message.timestamp - a.message.timestamp,
		)
		.slice(0, maxResults);
