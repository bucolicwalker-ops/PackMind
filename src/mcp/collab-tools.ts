...
const postMessageSchema = {
	...
	dogId: z
		.string()
		.min(1)
		.describe('Dog ID of the sender (e.g. "collie", "corgi", "gsd")'),
	content: z
		.string()
		.min(1)
		.describe(
			'Message content. @mentions are parsed automatically (e.g. "@铁铁 review this")',
		),
};
...
async function handlePostMessage(
	...
	if (resolvedMention) {
		...
		ballTracker.acquire(
			threadId,
			resolvedMention,
			`${dogEntry.config.nickname} passed ball via @mention`,
		);
...
const holdBallSchema = {
	...
	dogId: z
		.string()
		.min(1)
		.describe("Dog ID of the holder (must be current ball holder)"),
	reason: z
		.string()
		.min(1)
		.describe('Why the dog holds the ball (e.g. "waiting for CI to finish")'),
	nextStep: z
		.string()
		.min(1)
		.describe(
			'What the dog will do when re-invoked (e.g. "check CI result, then proceed")',
		),
	wakeAfterMs: z
		.number()
		.min(5000)
		.max(3600000)
		.describe("Delay in ms before system re-invokes (5000-3600000)"),
};

async function handleHoldBall(
	args: Record<string, unknown>,
): Promise<ToolResult> {
	const threadIdStr = args.threadId as string;
	const dogIdStr = args.dogId as string;
