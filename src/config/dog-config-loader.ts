...
import { DogCafeConfig, DogConfig, DogConfigEntry } from '../types/dog-breed.js';
...
export function loadDogConfig(filePath?: string): DogCafeConfig {
  const resolvedPath = filePath ? resolve(__dirname, filePath) : DEFAULT_CONFIG_PATH;
  ...
  for (const dog of config.dogs) {
    if (!dog.id || !dog.breedName || !dog.nickname || !dog.roleDescription) {
      throw new Error(`Dog entry missing required fields: ${JSON.stringify(dog)}`);
    }
  }
 ...
 */
export function findDogByMention(config: DogCafeConfig, text: string): { dogId: string; entry: DogConfigEntry } | undefined {
